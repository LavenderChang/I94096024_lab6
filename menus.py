...
# menu有兩種（兩層），一種是選塔，一種是升級
# 根據不同呼叫應該是在tower_group.py中），會叫出 UpgradeMenu或 BuildMenu中對應的buttons
...
class BuildMenu(Menu):
    def __init__(self, x, y):
        ...
        """
        #(X)負責把不同種類 tower 畫出來(class button會做這件事？？)
        #(O)回傳給 tower_group.py 的 add_tower
        self._buttons = [Button(RAPID_TEST_BTN_IMAGE, "rapid_test", self.rect.centerx, self.rect.centery - 70),
                           Button(ALCOHOL_BTN_IMAGE, "alcohol", self.rect.centerx, self.rect.centery + 75),
                           Button(PCR_BTN_IMAGE, "pcr", self.rect.centerx-65 , self.rect.centery+5), 
                         ]
